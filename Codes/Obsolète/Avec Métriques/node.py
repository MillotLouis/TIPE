...
# class Node:
#     def __init__(self,env,id,pos,initial_battery,max_dist,network:'network.Network'):
#         self.env = env
#         self.id = id
#         self.pos = pos
...
#         self.to_be_sent = defaultdict(list) #dest_id : data
...
#         else:
#             ## On vient de recevoir un RREP => on envoie tous les messages en attente
#             if rrep.src_id in self.to_be_sent:
#                 for msg in self.to_be_sent:
...
#     def handle_data(self,data:'network.Message'):
...
#         self.network.data_sent += 1
        
#         if dest_id in self.routing_table: # TTL des routes à implémenter ici
#             self.env.process(self.network.forward_data(self, msg))
#         else:
