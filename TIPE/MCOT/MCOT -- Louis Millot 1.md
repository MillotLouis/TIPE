...
---
Parfois les infrastructures nécessaires pour faire fonctionner un réseau d'appareil en passant par des points relais afin de faire communiquer les appareils indirectement sont absentes. C'est le cas lors de situations d'urgence (des catastrophes naturelles par exemple). La solution prépondérante pour pallier ce manque est la mise en place d'un réseau dit *ad-hoc*. Ceci consiste à faire communiquer les appareils directement entre eux au lieu de passer par un ou plusieurs point relais centraux. 
...
>(Note : Je ne sais pas si je devrais citer / détailler ces méthodes (\*) étant donné que je ne les utilise pas vraiment)
...
Soit les calculs sont plus légers mais reposent souvent sur des pondérations fixées de manière empirique ou générique \[3\] n'étant donc pas nécessairement optimaux dans ce cas d'application.
Pour pallier cela, des méthodes permettant de calculer dynamiquement les meilleurs pondérations ont été mises au point \[3\]. Mais ces calculs sont eux aussi lourds et complexes pour des systèmes embarqués tels que ceux présentés ici.

L'enjeu ici est donc de déterminer dans ce cas les pondérations optimales pour le protocole de routage modifié afin de garder une légèreté en calculs tout en améliorant l'efficacité grâce à une phase de simulation préalable ayant déterminé les coefficients optimaux.

### Problématique retenue
---
Comment l'intégration et l'optimisation paramétrique d'une métrique de routage dans le protocole AODV permet-elle d'optimiser l'utilisation des batteries ? en augmenter la durée de vie ?
